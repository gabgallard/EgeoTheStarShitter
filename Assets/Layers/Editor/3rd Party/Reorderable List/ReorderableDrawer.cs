...
namespace ABXY.Layers.ThirdParty.Malee.List {
	...
	public class ReorderableDrawer : PropertyDrawer {
		...
		public static ReorderableList GetList(SerializedProperty property, ReorderableAttribute attrib, int id, string arrayPropertyName) {
			...
			SerializedObjectTree sot = new SerializedObjectTree(property.serializedObject);
			SerializedPropertyTree incomingSerializedProperty = sot.FindProperty(property.propertyPath);
			SerializedPropertyTree array = incomingSerializedProperty.FindPropertyRelative(arrayPropertyName);
		...
		private struct SurrogateCallback {
			...
			internal void SetReference(SerializedPropertyTree element, Object objectReference, ReorderableList list) {

				SerializedPropertyTree prop = !string.IsNullOrEmpty(property) ? element.FindPropertyRelative(property) : null;
