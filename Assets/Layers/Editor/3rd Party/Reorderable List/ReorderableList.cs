...
namespace ABXY.Layers.ThirdParty.Malee.List {
	...
	public class ReorderableList {
		...
		public delegate void DrawElementDelegate(Rect rect, SerializedPropertyTree element, GUIContent label, bool selected, bool focused);
		...
		public delegate float GetElementHeightDelegate(SerializedPropertyTree element);
		public delegate float GetElementsHeightDelegate(ReorderableList list);
		public delegate string GetElementNameDelegate(SerializedPropertyTree element);
		public delegate GUIContent GetElementLabelDelegate(SerializedPropertyTree element);
		public delegate void SurrogateCallback(SerializedPropertyTree element, Object objectReference, ReorderableList list);
		...
		private SerializedPropertyTree list;
		...
		public ReorderableList(SerializedPropertyTree list)
			: this(list, true, true, true) {
		}

		public ReorderableList(SerializedPropertyTree list, bool canAdd, bool canRemove, bool draggable)
			: this(list, canAdd, canRemove, draggable, ElementDisplayType.Auto, null, null, null) {
		}

		public ReorderableList(SerializedPropertyTree list, bool canAdd, bool canRemove, bool draggable, ElementDisplayType elementDisplayType, string elementNameProperty, Texture elementIcon)
			: this(list, canAdd, canRemove, draggable, elementDisplayType, elementNameProperty, null, elementIcon) {
		}

		public ReorderableList(SerializedPropertyTree list, bool canAdd, bool canRemove, bool draggable, ElementDisplayType elementDisplayType, string elementNameProperty, string elementNameOverride, Texture elementIcon) {
			...
			if (list == null) {
				...
				SerializedPropertyTree array = list.FindPropertyRelative("array");
		...
		public SerializedPropertyTree List {
		...
		public int Length {
			...
			get {
				...
				//array size. So we have to find it. Not that great since we're creating SerializedObjectTrees here. There has to be a better way!

				int smallerArraySize = list.arraySize;

				foreach (Object targetObject in list.serializedObject.targetObjects) {

					SerializedObjectTree serializedObject = new SerializedObjectTree(new SerializedObject(targetObject));
					SerializedPropertyTree property = serializedObject.FindProperty(list.propertyPath);
		...
		public SerializedPropertyTree AddItem<T>(T item) where T : Object {

			SerializedPropertyTree property = AddItem();
		...
		public SerializedPropertyTree AddItem() {
		...
		public void RemoveItem(int index) {
			...
			if (index >= 0 && index < Length) {
				...
				SerializedPropertyTree property = list.GetArrayElementAtIndex(index);
		...
		public SerializedPropertyTree GetItem(int index) {
		...
		public int IndexOf(SerializedPropertyTree element) {
			...
			if (element != null) {
				...
				while (--i > -1) {
					...
					if (SerializedPropertyTree.EqualContents(element, list.GetArrayElementAtIndex(i))) {
		...
		/*
		private Rect GetElementHeaderRect(SerializedPropertyTree element, Rect elementRect) {
		...
		*/
		...
		private Rect GetElementRenderRect(SerializedPropertyTree element, Rect elementRect) {
		...
		private void SortElements(Rect rect, bool descending) {
			...
			SerializedPropertyTree prop = list.GetArrayElementAtIndex(0);

			if (prop.propertyType == SerializedPropertyType.Generic) {

				GenericMenu menu = new GenericMenu();

				SerializedPropertyTree property = prop.Copy();
				SerializedPropertyTree end = property.GetEndProperty();

				bool enterChildren = true;

				while (property.NextVisible(enterChildren) && !SerializedPropertyTree.EqualContents(property, end)) {
		...
		private void UpdateElementRects(Rect rect, Event evt) {
			...
			if (evt.type == EventType.Repaint) {
				...
				for (i = start; i < end; i++) {
					...
					SerializedPropertyTree element = list.GetArrayElementAtIndex(i);
		...
		private void DrawElement(SerializedPropertyTree element, Rect rect, bool selected, bool focused) {
		...
		private GUIContent GetElementLabel(SerializedPropertyTree element, bool allowElementLabel) {
		...
		private static string GetElementName(SerializedPropertyTree element, string nameProperty, string nameOverride) {
			...
			SerializedPropertyTree prop = element.FindPropertyRelative(nameProperty);
		...
		private void HandleSingleContextClick(Event evt, SerializedPropertyTree element) {
		...
		private void HandleDelete(object userData) {
			...
			selection.Delete(userData as SerializedPropertyTree);
		...
		private void HandleDuplicate(object userData) {
			...
			selection.Duplicate(userData as SerializedPropertyTree);
		...
		private void UpdateDragList(float dragPosition, int start, int end) {
			...
			for (int i = start; i < end; i++) {
				...
				SerializedPropertyTree property = list.GetArrayElementAtIndex(i);
		...
		private void ApplyReorder() {
			...
			list.serializedObject.UpdateIfRequiredOrScript();
		...
		private bool IsElementExpandable(SerializedPropertyTree element) {
		...
		struct DragList {
			...
			internal void RestoreState(SerializedPropertyTree list) {
		...
		struct DragElement {
			...
			internal SerializedPropertyTree property;
			internal int startIndex;
			...
			internal void RecordState() {
				...
				Iterate(this, property, (DragElement e, SerializedPropertyTree p, int index) => {

					e.states[index] = p.isExpanded;
				});
			}

			internal void RestoreState(SerializedPropertyTree property) {
			...
			private static void Iterate(DragElement element, SerializedPropertyTree property, System.Action<DragElement, SerializedPropertyTree, int> action) {

				SerializedPropertyTree copy = property.Copy();
				SerializedPropertyTree end = copy.GetEndProperty();

				int index = 0;

				while (copy.NextVisible(true) && !SerializedPropertyTree.EqualContents(copy, end)) {
		...
		class ListSelection : IEnumerable<int> {
			...
			internal bool CanRevert(SerializedPropertyTree list) {
			...
			internal void RevertValues(object userData) {
				...
				SerializedPropertyTree list = userData as SerializedPropertyTree;

				for (int i = 0; i < Length; i++) {

					SerializedPropertyTree property = list.GetArrayElementAtIndex(this[i]);
			...
			internal void Duplicate(SerializedPropertyTree list) {
			...
			internal void Delete(SerializedPropertyTree list) {
				...
				while (--i > -1) {
					...
					list.DeleteArrayElementAtIndex(this[i]);
		...
		static class ListSort {
			...
			private delegate int SortComparision(SerializedPropertyTree p1, SerializedPropertyTree p2);

			internal static void SortOnProperty(SerializedPropertyTree list, int length, bool descending, string propertyName) {

				BubbleSort(list, length, (p1, p2) => {

					SerializedPropertyTree a = p1.FindPropertyRelative(propertyName);
					SerializedPropertyTree b = p2.FindPropertyRelative(propertyName);
			...
			internal static void SortOnType(SerializedPropertyTree list, int length, bool descending, SerializedPropertyType type) {
			...
			private static void BubbleSort(SerializedPropertyTree list, int length, SortComparision comparision) {

				for (int i = 0; i < length; i++) {

					SerializedPropertyTree p1 = list.GetArrayElementAtIndex(i);

					for (int j = i + 1; j < length; j++) {

						SerializedPropertyTree p2 = list.GetArrayElementAtIndex(j);
			...
			private static int Compare(SerializedPropertyTree p1, SerializedPropertyTree p2, bool descending, SerializedPropertyType type) {
		...
		public struct Surrogate {
			...
			public void Invoke(SerializedPropertyTree element, Object objectReference, ReorderableList list) {
		...
		static class Internals {
			...
			static Internals() {
				...
				appendDragDrop = typeof(SerializedPropertyTree).GetMethod("AppendFoldoutPPtrValue", BindingFlags.NonPublic | BindingFlags.Instance);
			}

			internal static Object ValidateObjectDragAndDrop(Object[] references, SerializedPropertyTree property, System.Type type, bool exactType) {
				...
				dragDropValidationParams[2] = property.serializedProperty;
			...
			internal static void AppendDragAndDropValue(Object obj, SerializedPropertyTree list) {
