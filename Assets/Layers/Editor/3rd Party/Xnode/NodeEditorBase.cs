...
namespace ABXY.Layers.Editor.ThirdParty.Xnode {
	...
	public abstract class NodeEditorBase<T, A, K> where A : Attribute, NodeEditorBase<T, A, K>.INodeEditorAttrib where T : NodeEditorBase<T, A, K> where K : ScriptableObject {
		...
		public K target;
		public SerializedObjectTree serializedObject;
#if ODIN_INSPECTOR
		...
		public static T GetEditor(K target, NodeEditorWindow window) {
			...
			if (!editors.TryGetValue(target, out editor)) {
				...
				editor.serializedObject = new SerializedObjectTree(target);
			...
			if (editor.serializedObject == null) editor.serializedObject = new SerializedObjectTree(target);
			return editor;
		}
