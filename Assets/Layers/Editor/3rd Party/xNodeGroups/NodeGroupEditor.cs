...
using ABXY.Layers.Runtime.ThirdParty.XNode.NodeGroups;
using ABXY.Layers.Runtime.ThirdParty.XNode.Scripts;
using ABXY.Layers.Editor.Node_Editors;

namespace ABXY.Layers.Editor.ThirdParty.Xnode.NodeGroups {
	[CustomNodeEditor(typeof(NodeGroup))]
	public class NodeGroupEditor : FlowNodeEditor {
		...
		public override void OnBodyGUI() {
			...
			switch (e.type) {
				...
				case EventType.Repaint:
					...
					// Add scale cursors
					if (NodeEditorWindow.current.nodeSizes.TryGetValue(target, out size)) {
						Rect lowerRight = new Rect(target.position, new Vector2(15, 15));
						lowerRight.y += size.y - 19;
						lowerRight.x += size.x - 19;
			...
			GUI.DrawTexture(new Rect(group.width - 26, group.height + 25, 15, 15), corner);
		}
