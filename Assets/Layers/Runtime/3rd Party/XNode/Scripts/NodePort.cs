...
namespace ABXY.Layers.Runtime.ThirdParty.XNode.Scripts {
    [Serializable]
    public class NodePort {
        ...
        public bool CanConnectTo(NodePort port) {
            ...
            if (input.typeConstraint == Node.TypeConstraint.Inherited && !AreCastable(input.ValueType, output.ValueType)) return false;
            if (input.typeConstraint == Node.TypeConstraint.Strict && input.ValueType != output.ValueType) return false;
            if (input.typeConstraint == Node.TypeConstraint.InheritedInverse && !AreCastable(output.ValueType, input.ValueType)) return false;
            // Check output type constraints
            if (output.typeConstraint == Node.TypeConstraint.Inherited && !AreCastable(input.ValueType, output.ValueType)) return false;
            if (output.typeConstraint == Node.TypeConstraint.Strict && input.ValueType != output.ValueType) return false;
            if (output.typeConstraint == Node.TypeConstraint.InheritedInverse && !AreCastable(output.ValueType, input.ValueType)) return false;
            // Success
            return true;
        }
